...
There is a reference [docker-compose.yaml](https://github.com/paion-data/Kugelblitz/blob/master/docker-compose.yaml) where
...
Next we will verify the data has been saved into database. The quick start uses [ArangoDB](https://arango.paion-data.dev/)
...
[collection](https://arango.paion-data.dev/stable/concepts/data-structure/collections/) -> click __Content__ in the top
navigation bar. As we can see the data already exists as an
[Arango document](https://arango.paion-data.dev/stable/concepts/data-structure/documents/) in the database:
